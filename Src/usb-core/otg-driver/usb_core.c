...
#ifdef MY_DEBUG_USB
	printf("         USB_OTG_WritePacket ch_ep_num [%x] <-- : ",len);
	for(uint32_t ii=0; ii< len; ii++)
		printf("%0.2x ",(uint8_t)src[ii]);
	printf("\n");
#endif
...
#ifdef MY_DEBUG_USB
	printf("         USB_OTG_ReadPacket --> : ");
	for(uint32_t ii=0; ii< len; ii++)
		printf("%0.2x ",(uint8_t)dest[ii]);
	printf("\n");
#endif
...
USB_OTG_STS USB_OTG_EP0StartXfer(USB_OTG_CORE_HANDLE *pdev , USB_OTG_EP *ep)
{
	...
	if (ep->is_in == 1)
	{
		...
		MY_DEBUGF(("    USB_OTG_EP0StartXfer IN ep->xfer_len=%x size=%x ctn=%x\n",
	...
	{
		MY_DEBUGF(("    USB_OTG_EP0StartXfer OUT ep->xfer_len=%x size=%x ctn=%x\n",
						ep->xfer_len , deptsiz.b.xfersize , deptsiz.b.pktcnt));
		/* OUT endpoint */
